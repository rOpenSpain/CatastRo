...
**CatastRo** provides access to API services from the
[Spanish Cadastre](https://www.sedecatastro.gob.es/). With **CatastRo**, you can
download spatial objects such as buildings, cadastral parcels and maps, and
...
The SSL certificate of the Spanish Cadastre may cause
...
the **tibble** package. See
`vignette("ovcservice", package = "CatastRo")` for a detailed description of
these functions.

### INSPIRE

These functions return spatial objects in formats provided by the **sf** or
...
restrictions on the extent and number of elements that can be queried. For batch
...
When this option is set, **CatastRo** will look for cached files and load them,
...
Check the GitHub page for the
[source code](https://github.com/ropenspain/CatastRo/).
