...
package **tibble**. See `vignette("ovcservice")`, where these functions are
described.
...
These functions are named `catr_atom_xxx` where `_xxx` can have the values;
...
Note that there are some limitations on the extension and number of elements to
query. For batch-downloading the ATOM service is preferred.

These functions are named `catr_wms_xxx` where `_xxx` can have the values;
...
elements. The result is a raster on the format provides by **terra**.
