...
-   The `./vignettes` folder, including all subfolders with the following
    caveat:
    -    If you find two files with the same name but different extensions
        (e.g., `a_file.qmd` and `a_file.qmd.orig`), review only the `.orig`
        file, as the other file is generated from it.
