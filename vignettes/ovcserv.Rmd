...
The function `catr_ovc_rccoor()` receives the coordinates (*lat* and *lon*) and
...
`catr_ovc_cpmrc()` a cadastral reference (`rc`), the province (`province`) and
the town (`municipality`), `catr_ovc_cpmrc()` returns its coordinates *lat* and
...
`catr_ovc_cpmrc()` will throw a message and would return a tibble with no data.


```r
direction <- catr_ovc_cpmrc(
  rc = "13077A01800039",
  municipality = "SANTA CRUZ DE MUDELA"
) %>%
  knitr::kable()
