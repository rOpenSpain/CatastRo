---
title: "Get started"
description: Access Spanish Cadastre data with CatastRo.
vignette: >
...
retrieve addresses, buildings, cadastral parcels and georeferenced map images.
...
coordinates (reverse geocoding). **CatastRo** returns the results as tibbles.
These operations are described in detail in the
...
information across Europe and assists in policy-making across boundaries.
...
The Spanish Cadastre implementation of the INSPIRE Directive (see [Spanish
Cadastre INSPIRE
services](https://www.catastro.hacienda.gob.es/webinspire/index.html)) provides
access to spatial objects from the cadastral database:
...
We can also create thematic maps from attributes in the spatial objects. This
example visualizes the urban growth of Granada with **CatastRo**, reproducing a
map by [Dominic Royé](https://dominicroye.github.io) [@roye19] with the ATOM
service.

First, we retrieve the geometry of Granada's city center with **mapSpain**:
...
Next, we use `catr_get_code_from_coords()` to identify Granada's cadastral
municipality code, then download its buildings with
...
Finally, we group the data by construction year and create the visualization.
The `cut()` function creates classes for each decade from 1900 onward:
...
<div class="figure">
<img src="./dviz-1.png" alt="Figure 3: Granada - Urban growth" width="100%" />
<p class="caption">Figure 3: Granada - Urban growth</p>
</div>
