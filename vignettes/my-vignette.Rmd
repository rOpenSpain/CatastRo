...
```{r}
reference <- get_rc(lat = 38.6196566583596, lon =  -3.45624183836806,SRS = 'EPSG:4230', sleep_time = 10)
print(reference)
``` 
...
```{r}
reference <- get_rc(lat =  38.6196566583596, lon =-3.45624183836806, sleep_time = 10)
print(reference)
``` 
...
```{r}
references <- near_rc(lat = 40.96002, lon = -5.663408, SRS = 'EPSG:4230', sleep_time = 10)
print(references)
``` 

### Query CPMRC 

The opposite query is possible as well. Being given to the function `get_coor()` a cadastral reference (*RC*), the province (*Province*) and the  town (*Municipality*), `get_coor` returns its coordinates *lat* and *lon* in a particular *SRS* besides the direction (town, street and number).

```{r}
direction <- get_coor(RC = '13077A01800039', 
                      SRS = 'EPSG:4230',
                      Province = 'CIUDAD REAL', 
                      Municipality = 'SANTA CRUZ DE MUDELA', sleep_time = 10)
print(direction)
```

The argument *SRS* could be missed, in that case, `get_coor` returns the coordinates with which was stored.
...
```{r error=TRUE}
direction <- get_coor(RC = '13077A01800039',  Municipality = 'SANTA CRUZ DE MUDELA', sleep_time = 10)
direction <- get_coor(RC = '13077A01800039', sleep_time = 10)
print(direction)
```
...
test_that("given Municipio, Provincia is needed", {
  result <- get_coor(RC = '13077A01800039',SRS = 'EPSG:4230', Municipio = 'SANTA CRUZ DE MUDELA', sleep_time = 10)
