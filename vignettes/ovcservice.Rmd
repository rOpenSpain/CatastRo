...
Moreover, it is not necessary to be the owner to get the information; in fact, it is
...
purpose, the API asks for the longitude and the latitude. Furthermore, it
allows choosing the spatial reference system (SRS, also known as CRS) between a
...
These functions are named `catr_ovc_*` and return a `tibble`, as provided by
the package **tibble**.
...
argument `province` must be passed. If a value is passed to the
