test_that("catr_atom_get_address_db_to() returns NULL without data", {
...
test_that("catr_atom_get_address_db_all() returns NULL when offline", {
...
test_that("catr_atom_get_address_db_to() returns NULL when offline", {
  local_mocked_bindings(is_online_fun = function(...) {
    FALSE
  })
...
test_that("catr_atom_get_address_db_all() returns NULL after an HTTP 404", {
...
test_that("catr_atom_get_address_db_to() returns NULL after an HTTP 404", {
...
test_that("catr_atom_get_address_db_to() ranks office matches", {
  skip_on_cran()
  skip_if_offline()
  ...
  expect_silent(
    nmel <- catr_atom_get_address_db_to(to = "Melilla", cache_dir = cdir)
  )
  ...
  expect_silent(
    pal <- catr_atom_get_address_db_to(to = "Palencia", cache_dir = cdir)
  )
  ...
  # full name
  expect_silent(
    val <- catr_atom_get_address_db_to(to = "valencia", cache_dir = cdir)
  )
  expect_false(pal$munic[1] == val$munic[1])
})

test_that("address database functions warn about deprecated cache arguments", {
  cdir <- withr::local_tempdir(pattern = "testthat_ex2to")
  local_mocked_bindings(
...
test_that("catr_atom_get_address_db_to() handles a failed cached request", {
  skip_on_cran()
  skip_if_offline()
