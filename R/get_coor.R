...
#'
#'@description Returns the coordinate of a state in a SRS given the Province, Municipality 
...
#'@param SRS The Spatial Reference System desired for the coordinates, if it is missed, the
#'           returned coordinates will be given in the Spatial Reference System used by
#'           Googlemaps ('EPSG:4326'). 
#'           
...
#'@param Province A string with the Province of the state. It is not case sensitive.
#'@param Municipality A string with the Municipality of the state. It is not case sensitive.
#'
#'@return A list consisting on two numeric values: the coordinates (latitude, longitude) and
#'the reference system in which they are expressed.
#'        
...
get_coor <- function(RC, SRS= 'Google', Province = '',Municipality = ''){
  ...
  # REQUEST
  res <- GET(url = 'http://ovc.catastro.meh.es/ovcservweb/OVCSWLocalizacionRC/OVCCoordenadas.asmx/Consulta_CPMRC',
