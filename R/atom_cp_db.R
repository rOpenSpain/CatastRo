...
#'
#' - `catr_atom_get_parcels_db_all()` provides a top-level table including information
#'    of all the territorial offices (except Basque Country and Navarre) listing
#'    the municipalities included on each office.
#' - `catr_atom_get_parcels_db_to()` provides a table for the specified territorial
...
#' - `catr_atom_get_parcels_db_all()` provides a tibble with the following fields:
...
#' - `catr_atom_get_parcels_db_to()` provides a tibble with the following fields:
#'   - `munic`: Name of the municipality.
...
#' }
catr_atom_get_parcels_db_all <- function(cache = TRUE,
                                         update_cache = FALSE,
                                         cache_dir = NULL,
                                         verbose = FALSE) {
  api_entry <- paste0(
